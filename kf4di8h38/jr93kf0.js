...
checkLogin();
function checkLogin(){
	// jr93kf0 modified for admin only
...
function postClip(isFromClip = false) {
    ...
    if (!isNaN(convertedStart) && convertedEnd > 0) {
        ...
        notifier.warning("Clip length is too short.")
    }
}
...
function onEndChange() {
    ...
    if (!isNaN(enVal) && !isNaN(stVal) && $(enEl).val().length === 8){
        if (enVal <= stVal){
            $(enEl).val($(stEl).val());
            notifier.warning("End time must be greater then start");
            return;
        }
...
async function initWaveSurfer() {
    ...
    wavesurfer.on("audioprocess", (e) => {
        ...
        document.getElementById("runAudioTime").innerHTML = new Date(newTime * 1000)
            .toISOString()
            .substr(11, 8);
        currentTimeStamp = new Date(tourCoordinates[0].properties.time).getTime()+(newTime*1000);
        featureOnCurrentTimeStamp = tourCoordinates.find(f=>Math.round(new Date(f.properties.time).getTime()/1000) === Math.round(currentTimeStamp/1000))
...
async function fetchTours() {
	// jr93kf0 _all for admin only
    const response = await fetch(`${apiServer}index_all.cfm?tour=${nameKey}`, {method: "GET", credentials: "include", cache: "no-cache"});
    if (response.ok) {
    ...
    if (response.ok) {
        ...
        features.forEach((feature) => {
			if (typeof starttimelapsed === 'undefined'){ 
				starttimelapsed = feature.properties.time;
			}
            const coords = feature.geometry.coordinates;
			const secnds = ((Date.parse(feature.properties.time) - Date.parse(starttimelapsed))/1000);
...
var success = function () {
    ...
    tourJson2.CLIPS.forEach(function (item, index) {
        ...
        // Start and End Date
        template += '<div class="timeblock" style="display: flex; margin-left: 0.5rem; margin-bottom: 0.2rem">';
        template += '<button onclick="setClipTime(true)" class="setClipTime-left"> { </button>';
	template += '<input value="' + covert_time(item.START) + '" initvalue="' + covert_time(item.START) + '" id="startdate' + index + '" onkeyup="onStartChange()" placeholder="00:00:00" class="rounded start_time html-duration-picker" />';
        template += '<input value="' + covert_time(item.END)   + '" initvalue="' + covert_time(item.END)   + '" id="enddate'   + index + '" onkeyup="onEndChange()" placeholder="00:00:00"  class="rounded end_time html-duration-picker" />';
        template += '<button onclick="setClipTime(false)" class="setClipTime-right">}</button>';
        template += "</div>";

        // Transcription
        template += '<div class="form-group" style="margin-left: 3%; margin-bottom: 0.5rem">';
        template += '<textarea id="clip-transcription-'+index+'" class="form-control transcript"></textarea>';
        ...
        template += "</div>" +
            ...
            '                <div style="margin-left: 1rem">smooth line</div>' +
                                `<i
                                    style="margin-left: 1rem; margin-top: 0.3rem; cursor: pointer"
                                    class="fa fa-copy fa-regular fa-inverse"
